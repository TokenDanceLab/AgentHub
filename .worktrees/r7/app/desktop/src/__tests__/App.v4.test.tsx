...
import { useHubContacts, useHubWorkspaceProjects, useCreateHubWorkspaceProject, useUpdateHubWorkspaceProject } from '@/api/hubQueries';
...
vi.mock('@/api/hubQueries', () => ({
  useHubContacts: vi.fn(),
  useHubWorkspaceProjects: vi.fn(),
  useHubSearchUser: vi.fn(),
  useHubSendFriendRequest: vi.fn(),
  useHubAcceptFriendRequest: vi.fn(),
  useHubRejectFriendRequest: vi.fn(),
  useHubRemoveContact: vi.fn(),
  useHubBlockContact: vi.fn(),
  useHubUnblockContact: vi.fn(),
  useHubUpdateContactRemark: vi.fn(),
  useHubCreateContactGroup: vi.fn(),
  useCreateHubWorkspaceProject: vi.fn(),
  useUpdateHubWorkspaceProject: vi.fn(),
  useHubDocuments: vi.fn(),
  useCreateHubDocument: vi.fn(),
}));
...
const mockedUseHubContacts = vi.mocked(useHubContacts);
const mockedUseHubWorkspaceProjects = vi.mocked(useHubWorkspaceProjects);
const mockedUseCreateHubWorkspaceProject = vi.mocked(useCreateHubWorkspaceProject);
const mockedUseUpdateHubWorkspaceProject = vi.mocked(useUpdateHubWorkspaceProject);
...
describe('Desktop App v4 root', () => {
  beforeEach(() => {
    ...
    mockedUseHubContacts.mockReturnValue({
      data: undefined,
    } as ReturnType<typeof useHubContacts>);
    mockedUseHubWorkspaceProjects.mockReturnValue({
      data: undefined,
    } as ReturnType<typeof useHubWorkspaceProjects>);
    mockedUseCreateHubWorkspaceProject.mockReturnValue({
      mutateAsync: vi.fn(),
      isPending: false,
    } as ReturnType<typeof useCreateHubWorkspaceProject>);
    mockedUseUpdateHubWorkspaceProject.mockReturnValue({
      mutateAsync: vi.fn(),
      isPending: false,
    } as ReturnType<typeof useUpdateHubWorkspaceProject>);
    mockedUseHubStore.mockImplementation((selector) => selector({ authenticated: true }));
  ...
  it('enters a clean Desktop demo workbench from the login card', () => {
    ...
    // Demo mode with Edge online writes 'observed' (connect to Edge seed data)
