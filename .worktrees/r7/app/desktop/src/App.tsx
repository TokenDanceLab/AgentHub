...
export function DesktopWorkbenchApp({ onLogout }: DesktopWorkbenchAppProps = {}) {
  ...
  // Contacts: Hub contact mutations for the contacts page.
  const hubContactsEnabled = !workbench.isDemo;
  const searchUserMut = useHubSearchUser();
  const sendFriendReqMut = useHubSendFriendRequest();
  const acceptReqMut = useHubAcceptFriendRequest();
  const rejectReqMut = useHubRejectFriendRequest();
  const removeContactMut = useHubRemoveContact();
  const blockContactMut = useHubBlockContact();
  const updateRemarkMut = useHubUpdateContactRemark();
  const createContactGroupMut = useHubCreateContactGroup();

  const contactsActions = hubContactsEnabled ? {
    onSearchUser: async (query: string) => { await searchUserMut.mutateAsync(query); },
    onSendFriendRequest: async (userId: string, message?: string) => {
      await sendFriendReqMut.mutateAsync({ userId, message });
    },
    onAcceptRequest: async (requestId: string) => { await acceptReqMut.mutateAsync(requestId); },
    onRejectRequest: async (requestId: string) => { await rejectReqMut.mutateAsync(requestId); },
    onRemoveContact: async (userId: string) => { await removeContactMut.mutateAsync(userId); },
    onBlockContact: async (userId: string) => { await blockContactMut.mutateAsync(userId); },
    onUpdateRemark: async (userId: string, remark: string) => {
      await updateRemarkMut.mutateAsync({ userId, remark });
    },
    onCreateGroup: async (name: string, memberIds: string[]) => {
      await createContactGroupMut.mutateAsync({ name, memberIds });
    },
  } : undefined;
