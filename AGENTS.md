...
- 仓库只提交白名单 skill：`.codex/skills/set-goal/`、`.codex/skills/cross-review/`。
- 用户调用 `set-goal`，或要求持续推进、自我迭代、自己 review、交叉 review、worktree/subagent 分发时，必须先读 `.codex/skills/set-goal/SKILL.md`。
- 用户调用 `/cross-review`、`交叉审查`、`review 一下` 时，加载 `.codex/skills/cross-review/SKILL.md`。
...
- 本机 Agent 记忆和运行状态，例如 `.agenthub/memory/`、`.claude/`、`.codex/`；仓库级 `.codex/skills/set-goal/` 和 `.codex/skills/cross-review/` 是唯一例外。
