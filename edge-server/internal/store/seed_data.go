...
var seedThreads = []seedThreadDef{
	// 1. Builder — B0 SQLite 迁移 (richest thread, matching builderTranscript)
	{
		...
		Items: []seedItemDef{
			...
			{ID: "builder-msg-1", Type: "agent_message", Role: "assistant", SenderName: "Builder", Content: "收到，我会先做运行隔离和代码定位。这次任务会按真实执行流推进：建立 worktree、加载调试和验证 skill、读取消息块类型、搜索 SQLite/FTS 入口，再生成迁移草案。", RunID: "run-builder-1"},
			{ID: "builder-msg-2", Type: "agent_message", Role: "assistant", SenderName: "Builder", Content: "找到迁移边界。消息正文、工具调用、文件变更、审批、上下文使用和子 Agent 事件需要保留 block kind。搜索只走 text/code/status/citation 的摘要字段，其他结构保持 JSON。", RunID: "run-builder-1"},
			{ID: "builder-msg-3", Type: "approval", Role: "assistant", SenderName: "Builder", Content: "部署/写入审批：生成迁移 SQL 和导航 hook 更新，需要写入工作区文件。", RunID: "run-builder-1"},
			{ID: "builder-msg-4", Type: "agent_message", Role: "assistant", SenderName: "Builder", Content: "生成迁移草案。我会新增线程表、消息块表和 FTS shadow index。回滚点放在切换查询路径之前，避免旧历史消息不可读。", RunID: "run-builder-1"},
			{ID: "builder-msg-5", Type: "artifact", Role: "assistant", SenderName: "Builder", Content: "migrations/0007_chat_threads.sql — 86 行新增，创建 chat_threads 表和 FTS5 虚表。", RunID: "run-builder-1"},
			{ID: "builder-msg-6", Type: "diff", Role: "assistant", SenderName: "Builder", Content: "migrations/0007_chat_threads.sql\nhooks/useThreadNavigation.ts", RunID: "run-builder-1"},
			{ID: "builder-msg-7", Type: "agent_message", Role: "assistant", SenderName: "Builder", Content: "迁移方案已完成。产物包括迁移 SQL、线程导航 hook 调整、回滚说明和验证清单。下一步交给 Reviewer 做风险复核，确认后再由 Deployer 做预览发布。", RunID: "run-builder-1"},
		},
		Run: &seedRunDef{
			ID:     "run-builder-1",
			Status: "finished",
...
+- 导航 hook 不能改变现有 thread id。`,
	...
	// 2. Agent 协作群 — Hub 消息闭环 (matching projectGroupMessageLoopHubMessages)
	{
		...
		Items: []seedItemDef{
			...
			{ID: "collab-msg-1", Type: "agent_message", Role: "assistant", SenderName: "Builder", Content: "Reviewer，我会把 Hub message metadata 映射到 transcript，你复核 queued/assigned 可见性。", RunID: "run-collab-1"},
			...
			{ID: "collab-msg-2", Type: "agent_message", Role: "system", SenderName: "Orchestrator", Content: "task-a2a-review 已进入项目群 @Agent 队列，等待 Reviewer 接手。"},
			{ID: "collab-msg-3", Type: "agent_message", Role: "assistant", SenderName: "Builder", Content: "路由给 Reviewer 做 focused coverage，Builder 继续补 fixture contract。", RunID: "run-collab-1"},
			{ID: "collab-msg-4", Type: "agent_message", Role: "assistant", SenderName: "Reviewer", Content: "Reviewer 已完成 task-a2a-review，消息同步链进入 done。", RunID: "run-collab-1"},
		},
		Run: &seedRunDef{
			ID:     "run-collab-1",
			Status: "finished",
...
+}`,
	...
	// 4. Deployer — 静态预览已就绪
	{
		...
		Items: []seedItemDef{
			...
			{ID: "deployer-msg-1", Type: "agent_message", Role: "assistant", SenderName: "Deployer", Content: "静态预览已启动在 http://127.0.0.1:5176/desktop/，包含迁移 SQL diff、导航 hook 变更和风险说明。你可以在右侧栏浏览器 tab 里检查。", RunID: "run-deployer-1"},
			{ID: "deployer-msg-2", Type: "agent_message", Role: "assistant", SenderName: "Deployer", Content: "预览健康检查通过。构建产物 12 个文件、总大小 340KB。FTS5 虚表查询在 100ms 内返回。", RunID: "run-deployer-1"},
		},
		Run: &seedRunDef{
			ID:     "run-deployer-1",
			Status: "finished",
	...
	// 5. Orchestrator — 子任务分派 (seed run: finished)
	{
		...
		Items: []seedItemDef{
			...
			{ID: "orch-msg-1", Type: "agent_message", Role: "assistant", SenderName: "Orchestrator", Content: "已分派 4 个子任务：\n1. Builder — 梳理消息模型 + 输出迁移 SQL（进行中）\n2. Reviewer — 复核 schema 风险（等待中）\n3. Researcher — 调研 FTS5 性能基线（等待中）\n4. Deployer — 预览发布（阻塞，等 Builder 完成）", RunID: "run-orch-1"},
			{ID: "orch-msg-2", Type: "agent_message", Role: "assistant", SenderName: "Orchestrator", Content: "Builder 子任务状态：代码定位完成，正在生成迁移草案。预计 5 分钟内完成。Reviewer 已提前开始并行复核。", RunID: "run-orch-1"},
		},
		Run: &seedRunDef{
			ID:     "run-orch-1",
			Status: "finished",
...
+- 2 个建议（非阻塞）`,
	...
	// 7. Johnny — 人类协作者
	{
		...
		Items: []seedItemDef{
			...
			{ID: "johnny-msg-1", Type: "agent_message", Role: "assistant", SenderName: "Johnny", Content: "好的，你检查一下项目页的「进入工作区」按钮和侧栏私聊入口是否都正常跳转。有问题随时说。"},
		},
	},
	...
	// 8. Trump — 人类协作者
	{
		...
		Items: []seedItemDef{
			...
			{ID: "trump-msg-1", Type: "agent_message", Role: "assistant", SenderName: "Trump", Content: "收到，我来精简文档列表展示字段。只保留标题、状态和更新时间，其他详细信息移到详情页。"},
		},
	},
	...
	// 9. AI 游戏项目 — 项目群
	{
		...
		Items: []seedItemDef{
			...
			{ID: "ai-msg-1", Type: "agent_message", Role: "assistant", SenderName: "Orchestrator", Content: "AI 游戏项目已创建。团队成员：Delicious233（负责人）、Builder（开发）、Reviewer（审查）、Researcher（调研）、Deployer（发布）。", RunID: "run-ai-1"},
			{ID: "ai-msg-2", Type: "agent_message", Role: "assistant", SenderName: "Researcher", Content: "Researcher 已完成竞品调研报告，覆盖 8 款主流 AI 游戏引擎。建议采用 RL + LLM hybrid 方案。", RunID: "run-ai-1"},
		},
		Run: &seedRunDef{
			ID:     "run-ai-1",
			Status: "finished",
	...
	// 10. 文档重构 — 项目群
	{
		...
		Items: []seedItemDef{
			...
			{ID: "docs-msg-1", Type: "agent_message", Role: "assistant", SenderName: "Builder", Content: "文档重构已完成。变更摘要：\n- 新增 12 篇 API 文档\n- 统一中英文标题层级\n- 补充 4 个缺失的 error code 说明\n- 更新 README 中的快速开始指南", RunID: "run-docs-1"},
			{ID: "docs-msg-2", Type: "agent_message", Role: "assistant", SenderName: "Builder", Content: "所有文档已同步到 docs/ 目录。i18n parity 验证通过，中英文结构一致。", RunID: "run-docs-1"},
		},
		Run: &seedRunDef{
			ID:     "run-docs-1",
			Status: "finished",
