...
func TestWorkspaceAllowlistValidatePath(t *testing.T) {
	cases := []struct {
		...
		{"filesystem root allows descendants", "/", "/etc/passwd", true},
		{"filesystem root itself", "/", "/", true},
		{"windows backslashes", `C:\work`, `C:\work\file.txt`, true},
		{"windows drive letter case", `C:\work`, `c:/work/file.txt`, true},
		{"windows directory component case-insensitive", `C:\Work`, `c:\work\File.txt`, true},
		{"windows root itself case-insensitive", `C:\Work`, `c:\WORK`, true},
		{"windows drive root allows descendants", "C:/", "c:/work/file.txt", true},
		{"windows drive root rejects other drive", "C:/", "D:/work/file.txt", false},
		{"windows adjacent prefix stays rejected", `C:\Work`, `C:\Work-evil\file.txt`, false},
		{"path with interior space", "/workspace", "/workspace/my docs/file.txt", true},
		{"file with trailing space inside workspace", "/workspace", "/workspace/readme.txt ", true},
		{"leading space path rejected", "/workspace", " /workspace/file.txt", false},
		{"trailing-space workdir rejects trimmed bypass", "/work ", "/work/secret", false},
