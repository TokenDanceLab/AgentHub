...
// fakeAdapterRegistry implements the AdapterRegistry port with a fixed set
...
func newFakeAdapterRegistry(avail map[string]bool) *fakeAdapterRegistry {
	r := &fakeAdapterRegistry{adapters: make(map[string]AgentAdapter)}
	for id, ok := range avail {
		r.adapters[id] = &mockAgentAdapter{id: id, available: ok}
	}
	return r
}

func (r *fakeAdapterRegistry) Get(id string) (AgentAdapter, bool) {
	r.mu.Lock()
	defer r.mu.Unlock()
	a, ok := r.adapters[id]
	return a, ok
...
func (r *fakeAdapterRegistry) ListIDs() []string {
	r.mu.Lock()
	defer r.mu.Unlock()
