...
func TestHubCallbackEmitterKeepsTextWithoutTypedDoubleForward(t *testing.T) {
	...
	if len(cb.order) != 3 || cb.order[0] != "stream" || cb.order[1] != "typed:"+adapters.BusEventThinking || cb.order[2] != "done" {
		t.Fatalf("pending text must precede the typed boundary: %v", cb.order)
	}
