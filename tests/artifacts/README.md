...
| `manifest-<YYYYMMDD-HHMMSS>.json` | 真实 E2E lane evidence manifest（六字段合同：evidence_level / real_tested / claim / status / skipped_evidence_levels / planned_evidence_levels；外加 commit / scope provenance） | `scripts/e2e/run-real-e2e-lane.sh`（#1839 B3；`scripts/verify/verify-real-e2e-lane-manifest.py` 校验） | ✅ |
