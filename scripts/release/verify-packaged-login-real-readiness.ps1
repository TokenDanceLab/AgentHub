...
Assert-Contains "scripts\verify\verify-oidc-flow.ps1" '\[switch\]\$LocalOnly' "OIDC flow verifier exposes -LocalOnly"
...
Assert-Contains "app\desktop\src-tauri\src\host\edge.rs" "get_packaged_login_readiness" "Tauri command exposes packaged login readiness"
Assert-Contains "app\desktop\src-tauri\src\host\auth.rs" 'status: "proposal_only"\.to_string\(\)' "Tauri command keeps real packaged E2E proposal-only"
Assert-Contains "app\desktop\src-tauri\src\lib.rs" "crate::host::edge::get_packaged_login_readiness" "Tauri invoke handler registers packaged login readiness command"
Assert-Contains "scripts\release\verify-tauri-package-readiness.ps1" "Desktop version metadata" "Tauri package readiness gate exists"
Assert-Contains "scripts\release\verify-tauri-package-readiness.ps1" "Generated artifact ignore policy" "Tauri package gate blocks generated artifact drift"
...
Assert-Contains "docs\roadmap.md" "TokenDanceID/OIDC" "roadmap names the real OIDC login boundary"
Assert-Contains "docs\roadmap.md" "approval gate" "roadmap keeps real OIDC login approval-gated"
Assert-Contains "docs\roadmap.md" "OAuth client" "roadmap lists approved OAuth client as a prerequisite"
Assert-Contains "docs\roadmap.md" "test account" "roadmap lists disposable or pre-approved test account as a prerequisite"
Assert-Contains "docs\roadmap.md" "/client/auth/me" "roadmap lists auth-me proof in the evidence boundary"
Assert-Contains "docs\roadmap.md" "client secret.*cookie.*authorization" "roadmap bans secret-bearing evidence from docs/logs/screenshots/commits/reports"
Assert-Contains "docs\governance\governance-execution.md" "Packaged Desktop OIDC readiness.*proposal-only gate" "governance keeps real packaged E2E proposal-only"
Assert-Contains "docs\governance\governance-execution.md" "Packaged real login dry readiness.*Hub/TokenDance ID.*secrets" "governance records no live TokenDance ID or browser action"
