...
$dryGateText = Read-Text "scripts\release\verify-tauri-package-dry.ps1"
...
Assert-True ($dryGateText -match "AgentHub_\$\{desktopVersion\}_x64-portable\.zip" -or $dryGateText -match "portable\.zip") "release readiness dry gate names portable.zip artifact"
Assert-True ($dryGateText -match "setup\.exe") "release readiness dry gate collects NSIS setup.exe"
...
Assert-True ($dryGateText -match "RequireUpdaterMetadata" -and $dryGateText -match "not_produced_unsigned_build") "unsigned dry gate records updater metadata as a separate signing/release gate"
Assert-True ($checkerText -match "Assert-UpdaterLatestMetadata" -and $checkerText -match "latest\.json" -and $checkerText -match "\.sig") "built artifact mode checks latest.json and .sig when signing/release metadata is supplied"
...
Assert-True ($readinessWorkflowText -match "verify-tauri-package-dry\.ps1" -and $readinessWorkflowText -match "RunWindowsBundle" -and $readinessWorkflowText -match "ArtifactsRoot dist") "release readiness workflow delegates unsigned Windows package proof to the dry gate"
