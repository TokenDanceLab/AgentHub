...
Assert-LiteralNotContains "app/web/src/api/edgeClient.ts" "fetch(" "Web Edge compatibility stub has no network fetch"
Assert-PathMissing "app/web/src/api/edgeAuth.ts" "Web has no Edge auth helper"
Assert-PathMissing "app/web/src/api/eventClient.ts" "Web has no Local Edge event stream client"
Assert-PathMissing "app/web/src/hooks/useHubIntegration.ts" "Web has no Desktop-only Hub-Edge bridge hook"
Assert-PathMissing "app/web/src/hooks/useChatMessages.ts" "Web has no legacy Local Edge chat event reducer"
Assert-PathMissing "app/web/src/hooks/useEventStream.ts" "Web has no Local Edge event stream hook"
Assert-PathMissing "app/web/src/hooks/useEdgeStatus.ts" "Web has no Local Edge status hook"
Assert-PathMissing "app/web/src/hooks/useRunners.ts" "Web has no Local Edge runner hook"
Assert-Contains "scripts/verify-web-hub-boundary.ps1" "Web Hub-only boundary" "focused Web Hub-only boundary checker exists"
