...
def case_stale_cache_phantom(fixture: str) -> None:
    ...
    phantom = new_issue(
        "gosec",
        "../.worktrees/int-round-66-go/edge-server/internal/jwtutil/jwt_test.go",
        "G101: Potential hardcoded credentials",
        219,
    )
    write_json_file(os.path.join(fixture, "scripts/verify/edge-lint-baseline.json"), {"findings": []})
...
def case_stale_absolute_phantom(fixture: str) -> None:
    ...
    phantom = new_issue(
        "gosec",
        "/nonexistent/worktree/edge-server/internal/jwtutil/jwt_bench_test.go",
        "G101: Potential hardcoded credentials",
        18,
    )
    write_json_file(os.path.join(fixture, "scripts/verify/edge-lint-baseline.json"), {"findings": []})
...
def case_absolute_path_inside_module(fixture: str) -> None:
    ...
    real = os.path.join(fixture, "edge-server", "internal", "service", "agent_dispatch.go")
