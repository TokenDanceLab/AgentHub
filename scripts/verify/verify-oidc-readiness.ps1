...
Assert-Contains "hub-server/internal/service/oidc/oidc.go" "ParseTokenDanceJWT" "Hub validates TokenDance ID token"
Assert-Contains "hub-server/internal/service/oidc/oidc.go" "FindOrCreateByTokenDanceSub" "Hub maps tokendance_sub to Hub user"
Assert-Contains "hub-server/internal/service/oidc/oidc.go" "UpsertRefreshToken" "Hub issues Hub-local refresh session"
...
Assert-Contains "hub-server/internal/handler/ws_test.go" "TestWebSocketRouteRejectsHubLocalQueryTokenBeforeUpgrade" "Hub tests reject Hub-issued query token before WebSocket upgrade (query-token channel retired)"
