...
def get_live_fingerprints(report, module_dir: str) -> tuple:
    ...
    module_dir is the absolute path of the linted module (…/hub-server); the
    ...
    for issue in report.get("Issues", []):
        ...
        # ../.worktrees/int-round-66-go/hub-server/internal/jwtutil/, a directory that
...
def main() -> int:
    ...
    if not json_path:
        ...
        cache_dir = os.path.join(
            cache_root, f"golangci-lint-{hashlib.sha1(hub_dir.encode()).hexdigest()[:12]}"
        )
        lint_env = dict(os.environ, GOLANGCI_LINT_CACHE=cache_dir)
        lint_run = subprocess.run(lint_args, cwd=hub_dir, capture_output=True, text=True, env=lint_env)
    ...
    live, phantoms = get_live_fingerprints(report, os.path.abspath(hub_dir))
    if phantoms:
