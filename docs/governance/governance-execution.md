...
This file maps TokenDance system-level governance work into AgentHub-owned execution items. It does not replace `docs/roadmap.md`, `docs/product-requirements.md`, or `docs/system-architecture.md`; it tells agents which root queue IDs apply before they edit Hub, Edge, Desktop, Web, Feishu, or auth code.
...
|---|---|---|---|
| TD-P0-HUB-01 | Hub Server | `hub-server/`, `api/openapi.yaml`, `docs/system-architecture.md`, `docs/implementation-guide.md` | Hub-owned TokenDance ID callback/code exchange; issuer/audience/JWKS tests; `tokendance_sub` user mapping; Hub-local access/refresh session tests |
...
6. Private server evidence belongs in `C:\Users\Ding\server`, not in AgentHub docs.
