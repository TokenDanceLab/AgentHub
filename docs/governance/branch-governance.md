...
| 项 | 当前事实 |
|---|---|
| 主开发分支 | `dev/delicious233`，当前主 worktree HEAD 为 `45f9ba1` |
| Integration 分支 | `feat/team-integration-sweep` |
| Integration worktree | `.worktrees/team-integration-sweep` |
| 主 worktree | dirty，仍有 UIUX、OIDC、Web 相关并行改动；不要当作干净可发布基线 |
| Worktree 数量 | 当前 `git worktree list` 为 23 个，包含新 integration worktree |
| Web worktree | `.claude/worktrees/feat+web-desktop-parity` / `worktree-feat+web-desktop-parity` 仍在工作，不得写成清理态 |
| 迁移文件 | `hub-server/migrations` 当前有 28 个 `.up.sql`；integration 已将后续平台 migrations 重排到 `0022`-`0028`，仍需 schema/migration smoke |

## 当前分支状态

| 分支 | 说明 | 状态 |
|------|------|:--:|
| `dev/delicious233` | 主开发分支，当前 HEAD `45f9ba1` | 活跃 |
...
| `.claude/worktrees/feat+web-desktop-parity` | `worktree-feat+web-desktop-parity` | Web parity | 仍在工作 |
...
- 不要在公开文档中写本机绝对路径；使用 repo-relative path，例如 `.worktrees/team-integration-sweep`。
