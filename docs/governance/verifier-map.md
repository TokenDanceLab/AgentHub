...
| secrets/token 不落库（hard-blocking，无 `continue-on-error`；放行面只有 `*.env.example` 路径、占位值词表与 endpoint/path 形状，字面量规则要求 token 前缀在词边界上以避免良性 ID 误报；**没有** mock/test 凭据夹具的 allowlist，故含假凭据的脱敏测试文件不可搬家，见 #2295） | `scripts/verify/check-secrets.sh` | checks.yml → validate |
