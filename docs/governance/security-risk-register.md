...
| AH-SR-046 | High | Partial mitigated in repo (#444) | Edge `PostRuns` dual-token 校验已有；Hub 已实现 `jwtutil.IssueCapabilityToken` 并在 HTTP dispatch 附带 `X-AgentHub-Capability-Token`（env `AGENTHUB_JWT_SECRET` + device）。**purpose=`run-start` 已强制**（非 run-start → 403 + 负例 `TestRunStartDualToken_WrongPurposeReturns403`）。仍缺完整 action/target/workspace 绑定、CORS 全覆盖与 Hub→Edge 签发/校验 E2E 证据。 | 补齐 route-scoped action/target/workspace 绑定与 E2E 证据。 |
...
| AH-SR-049 | High | Partial mitigated in repo (#445) | Hub outbox + `StartDeliveryRetryLoop` 已接线；Edge 内存 `DeliveryJournal` + **可选 SQLite durable journal**（`AGENTHUB_DELIVERY_JOURNAL_DB` → `EnableSQLiteJournal` 双写；跨 open 持久化测试通过）。仍缺 idempotent Hub ack 合同、完整 replay/cursor reconciliation 与 E2E 证据。 | 补齐 sequence/idempotent ack、replay/cursor reconciliation 与 fixture/runtime 证据。 |
