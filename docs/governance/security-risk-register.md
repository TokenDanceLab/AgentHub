...
| AH-SR-049 | High | Partial mitigated in repo | Hub outbox + `StartDeliveryRetryLoop` 已接线；Edge 已有内存 `DeliveryJournal`（seq + Snapshot）接入 CallbackClient。仍缺跨进程 durable journal、idempotent Hub ack 合同、完整 replay/cursor reconciliation 与 E2E 证据。 | 补齐 Edge outbox/journal、sequence/idempotent ack、replay/cursor 与 reconciliation，并附 fixture/runtime 证据。 |
