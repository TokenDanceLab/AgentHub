...
## Documentation Consolidation Addendum

Operator request on 2026-06-27: keep active docs aggressively small and canonical. Do not solve doc drift by adding another active document. The cleanup order is:

1. `AGENTS.md` owns stable project rules and agent workflow.
2. `docs/progress/MASTER.md` owns current SPEC progress only.
3. `docs/roadmap.md` owns overall product/spec roadmap only.
4. `docs/architecture.md` and `docs/architecture/` own current architecture contracts.
5. Historical longform, old audits, release packets, stale plans, reference snapshots, and obsolete skills move to `docs/archive/` or `docs/archives/`.

Acceptance for this addendum is not "more documentation exists"; it is fewer active truth sources, shorter owner docs, and verifier coverage that blocks stale active references from returning.
...
|:--|:--|:--|:--|:--|:--|:--|:--|:--|:--|
| T1.1 | Normalize branch/worktree governance | P0 | M | - | A | S, U, R | Docs-only; run stale branch search and `git status --short --branch; git worktree list` | Update instruction surfaces if branch rules change | `AGENTS.md` owns branch/worktree rules; old branch-rule snapshots are archived and live-state commands are canonical. |
| T1.2 | Normalize document standards and active/archive rules | P0 | M | - | B | S, U, R | Docs-only; run stale doc-structure search and `git diff --check` | Update `AGENTS.md` if workflow rule changes | `AGENTS.md` and `docs/README.md` agree: active spec work uses `docs/progress/MASTER.md`; completed spec artifacts go to `docs/archives/`; stale long-lived docs are replaced or deleted, not duplicated. |
...
|:--|:--|:--|:--|:--|
| A | T1.1 | M | Medium | `AGENTS.md`, branch/worktree archive snapshots |
| B | T1.2 | M | Medium | `docs/README.md`, `AGENTS.md`, `scripts/verify-doc-ssot.ps1` |
...
| # | Task | Priority | Effort | Depends On | Lane | S.U.P.E.R | Test Expectation | Memory Impact | Acceptance Criteria |
|:--|:--|:--|:--|:--|:--|:--|:--|:--|:--|
| T2.1 | Make evidence-level matrix canonical | P0 | M | T1.2 | A | P, R | Docs-only plus stale "real" wording scan | Update `AGENTS.md` if canonical entry changes | `real-e2e-acceptance` remains the operational SOP; `AGENTS.md`, roadmap/architecture references, and release docs use one evidence vocabulary without duplicating the evidence matrix. |
...
| Lane | Tasks | Combined Effort | Merge Risk | Key Files |
|:--|:--|:--|:--|:--|
| A | T2.1 | M | Medium | `.agents/skills/real-e2e-acceptance/SKILL.md`, `AGENTS.md`, `scripts/verify-real-e2e-contract.ps1` |
...
| T3.0c | Sync progress SSOT and doc governance guardrails | P2 | S | T3.0b | Prep | S, U, R | Docs-only; run doc SSOT, project skill whitelist, real E2E contract, stale wording search, and diff check | Update MASTER/roadmap/verifier only; no new rule surface | `docs/progress/MASTER.md` and `docs/roadmap.md` match GitHub live state after #344; long session evidence is not duplicated in active entrypoints; verifier blocks stale branch/phase progress wording. |
| T3.0d | Aggressively consolidate active documentation | P0 | M | T3.0c | Prep | S, U, R | Docs-only; audit active Markdown inventory, archive/delete duplicates, run doc SSOT, project skill whitelist, real E2E contract, stale wording search, OpenAPI YAML parse, and diff check | Update `AGENTS.md` or verifier only if the owner rule changes; otherwise update MASTER/roadmap task status | Active docs follow the three-entry rule (`AGENTS.md`, `MASTER`, `roadmap`) plus architecture/API owner docs; giant or duplicate active docs are shortened or archived; legacy root rule files, stale skill rules, old screenshot/dev-team surfaces, and outdated Desktop/Web freeze/debug guidance do not reappear. |
| T3.0e | Degiant active documentation spine | P1 | M | T3.0d | Prep | S, U, R | Docs/governance task; write this plan before audit, inventory active Markdown/rule surfaces, archive/merge stale giants, then run doc SSOT, project skill whitelist, real E2E contract, stale wording, OpenAPI YAML, and diff checks | Update `AGENTS.md`, `MASTER`, `roadmap`, and guardrails only when durable rules change; no new active rule surface | Active docs are aggressively reduced to focused owner docs; obsolete or duplicate longform material is archived under the existing archive trees; second root rule files stay absent; `AGENTS.md`, `MASTER`, and `roadmap` remain the top-level rule/progress spine. |
...
| T3.2 | Align frontend architecture docs to shared implementation | P1 | M | T3.1 | A | S, U, R | Docs/source-reference scan + doc SSOT guard update; no broad snapshots | Update `AGENTS.md` or doc verifier only if the owner rules change | `docs/architecture.md` and `docs/architecture/04-frontend-data-flow.md` describe current shared workbench/chat/data-mode reality without stale ChatView phase banners; active docs stay as short owner entrypoints; duplicate or historical frontend/design evidence moves to archive instead of being appended. |
...
| Lane | Tasks | Combined Effort | Merge Risk | Key Files |
|:--|:--|:--|:--|:--|
| Prep | T3.0, T3.0b, T3.0c, T3.0d, T3.0e | M+M+S+M+M | Medium | `api/`, module README files, `hub-server/README.md`, `docs/architecture/`, `docs/governance/`, `docs/progress/MASTER.md`, `docs/roadmap.md`, `scripts/verify-doc-ssot.ps1` |
...
| # | Task | Priority | Effort | Depends On | Lane | S.U.P.E.R | Test Expectation | Memory Impact | Acceptance Criteria |
|:--|:--|:--|:--|:--|:--|:--|:--|:--|:--|
| T4.1 | Run focused acceptance gate bundle | P0 | L | T3.0, T3.0b, T3.0c, T3.0d, T3.0e, T3.1, T3.3, T3.4, T3.5 | A | P, E | Run selected docs/API/frontend/backend/visual/package/perf gates based on touched surfaces | None | Evidence table lists commands, pass/fail, artifact paths, and explicit not-tested boundaries. |
