...
P0 完整闭环已实现：本地项目、Thread、真实 Runner、Diff、Apply / Discard、Approval 和 Preview。
...
4. 用户输入 prompt 并选择 Agent（Claude Code / OpenCode / Codex）。
5. Runner 启动真实 Agent CLI，实时流式输出。
...
| P2 | Edge-Hub Sync | Edge 注册、设备状态、消息/事件同步、Web/Mobile 查看状态和远程审批 | 规划中（Q3） |
| P3 | Relay / Cloud | Hub 中继、Cloud Edge、Cloud Runner、远程 Preview、Artifact Proxy | 规划中（Q3） |
| P4 | 团队 IM | 用户、联系人、群组、团队空间、团队 Memory、组织权限 | 规划中 |
...
启动 Local Edge
-> 打开 Desktop UI
-> 选择 Agent（Claude Code / OpenCode / Codex）
-> 输入 prompt
