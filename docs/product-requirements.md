...
当前仓库处于 **客户端 M1 收口** 阶段。M1 只验证本地 mock 链路：

```text
Desktop UI -> Local Edge -> Mock Run -> WebSocket events -> UI EventLog
```

M1 不等于完整 P0。完整 P0 需要跑通本地项目、Thread、真实 Runner、Diff、Apply / Discard、Approval 和 Preview 的闭环。
...
| 小团队 | 像群聊一样组织多个 Agent 协作完成任务 | 能在同一个 Thread 中 @Agent，查看不同 Agent 的进度、产物和审查意见 |
| 比赛评审 | 快速看懂产品定位、技术路线、可演示流程和 AI 协作记录 | 能启动 Local Edge/Desktop，创建 mock run，看到 WebSocket 事件和 EventLog，并理解完整 P0 的未完成项 |
...
1. 启动 Local Edge。
2. 打开 Desktop Web UI。
3. UI 显示 Edge online/offline、Mock Runner、EventLog。
4. 用户点击 Start Mock Run。
5. Edge 发送 `run.queued`、`run.started`、`run.output.batch`、`run.finished`。
6. UI 通过 WebSocket 看到事件流。

### 完整 P0 演示流

1. 用户选择一个本地项目。
2. 新建 Thread，并 @ClaudeCode 或 @Codex 提交任务。
3. Edge 创建 AgentRun。
4. Runner 启动真实 Agent CLI。
5. UI 实时显示日志、进度、审批请求。
6. Runner 在隔离 workspace 或 worktree 中生成变更。
...
| Runner | Edge 能调度 Runner，Runner 能启动一个真实 Agent CLI |
| stdout/stderr | Runner 输出能以 Item 或 event 形式流式显示，并可回放 |
...
| Preview 面板 | Runner 或 Edge 能启动预览，并通过 UI 打开 |
...
| 要求 | 怎样算完成 |
|---|---|
| 本地优先 | 断网时 Desktop -> Local Edge -> Local Runner 的主流程仍可运行 |
...
| 路径保护 | Runner 只能在授权 workspace 内读写，不默认访问用户全盘 |
| adapter 可扩展 | Codex CLI、Claude Code、OpenCode 通过同一 Runner adapter 模型接入 |
...
Hub Network、Web/Mobile、团队账号和多人 IM 都是 P1+ 能力，不作为客户端 M1 或本地 P0 的验收条件。
...
-> 打开 Desktop UI
-> 创建 mock run
-> 看到 WebSocket events 和 EventLog
-> 说明完整 P0 还需要 Project/Thread/真实 Runner/Diff/Approval/Preview
```
