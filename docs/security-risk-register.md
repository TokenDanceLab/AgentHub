...
| AH-SR-019 | Medium | Mitigated in repo; deploy/client verification required | Auth, OIDC, and device-registration HTTP boundaries now normalize `device_id` as UUID before reaching service/repository code; Web legacy password login now sends `device_type=web`, so browser sessions satisfy `/web/*` without gaining `/edge/*` callback privileges. | `hub-server/internal/handler/auth.go:69`, `hub-server/internal/handler/device.go:35`, `hub-server/internal/handler/oidc.go:45`, `hub-server/internal/handler/validation.go:9`, `app/web/src/api/hubAuth.ts:307`, `app/web/src/api/hubAuth.test.ts:1`, `api/openapi.yaml:908` | Verify deployed Hub/Desktop/Web clients always send persisted UUID device IDs and that Web fallback login reaches `/web/*` while `/edge/*` remains desktop-only. |
...
- 2026-05-26: `AH-SR-019` Web fallback login device boundary was tightened in repo. Browser password fallback now requests a Hub `web` session instead of a `desktop` session, and OpenAPI documents `device_type`/UUID `device_id` as required for `/client/auth/login`.
