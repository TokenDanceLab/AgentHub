...
| ID | Severity | Status | Risk | Evidence | Next action |
|---|---|---:|---|---|---|
| AH-SR-012 | High | Open | No OIDC deployment evidence. Hub OIDC callback, JWKS validation, code exchange, and Hub session issuance have code and test coverage, but there is no production or staging deployment verification (callback URL registered, client secret configured, TLS active, token flow smoke-tested against real TokenDance ID). This is release-blocking. | `hub-server/internal/handler/oidc.go`, `hub-server/internal/service/oidc.go`, `hub-server/internal/service/oidc_test.go` | Deploy Hub to a reachable environment, register the OIDC client with TokenDance ID, run end-to-end OIDC smoke from a browser, and record evidence in private server docs. Do NOT copy live callback URLs, client secrets, or tokens into this public repo. |
