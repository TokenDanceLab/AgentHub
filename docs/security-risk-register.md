...
|---|---|---:|---|---|---|
| AH-SR-001 | High | Open | TokenDance ID bearer validation does not visibly enforce `iss` and `aud` before accepting RS256 tokens. A token minted for another relying party could authenticate to Hub if it is signed by the configured JWKS. | `hub-server/internal/jwtutil/tokendance.go:171`, `hub-server/internal/jwtutil/tokendance.go:180`, `README.md:235` | Validate issuer and AgentHub client audience, add wrong-audience tests, and sync root `../docs/identity-auth.md`. |
