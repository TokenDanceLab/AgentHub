...
推进 M2 Edge 本地数据层，把 M1 的内存事件流升级为 Project / Thread / Run / Item / Event 模型。当前 PR #30 已完成内存态最小模型，后续继续补持久化、message/item 写入和 Runner lifecycle。
...
- [ ] 补齐 `POST /v1/threads/{threadId}/messages` 到 Item / event 的写入链路。
