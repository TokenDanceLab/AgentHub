...
| 维度 | 状态 | 关键发现 |
|------|:--:|------|
| 设置页 | 危机 | SettingsPage.tsx 4200 行单体 + 29 个死代码 section 文件 |
| Chat/IM 展示 | 待完善 | ChatView 功能丰富但有 stub handler；IM 子组件全内联样式 |
| Web 页面 | 不可用 | 3/5 页面 100% mock 数据，零真实功能 |
| 设计系统 | 基础扎实但有裂缝 | token 架构好，但 797 个硬编码 rgba() 绕过 token |
| 测试覆盖 | 两极分化 | Desktop 56 文件 / Web 1 文件 / Shared 0 文件 |
| 后端 | 高度完整 | Hub 145+ 操作已实现，OIDC 全链路已通，缺浏览器冒烟 |
| 文档 | 基本齐全 | CHANGELOG 冻结在 v0.1.0，缺 Edge/Hub 本地开发指南 |
