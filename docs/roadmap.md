...
- [ ] Runtime history follow-up：Hub typed RunEvent 已有最小持久化和 owner-scoped read API；下一步是让 Web/Home 从 `agent.stream` / `GET /web/agent-tasks/{id}/events` 展示 run history、状态筛选、token/step/elapsed/approval/artifact 摘要，并把 Edge/adapter 的 tool/file/approval payload 统一到 AgentHub event shape。Edge 仍保留完整本地 EventStore，前端只消费 AgentHub 事件族，不直读 Codex/Claude Code/OpenCode 私有 JSON。bytedance.md 功能差距和竞品方向见 `docs/reference/cross-comparison/11-bytedance-feature-map.md`。
