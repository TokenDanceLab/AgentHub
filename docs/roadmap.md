...
> 最后更新: 2026-06-05 (R2 Desktop gate 已绿 + 深色任务队列参考收敛) | 唯一事实源 | 旧版归档: [archive/roadmap-v2-pre-restructure-20260605.md](archive/roadmap-v2-pre-restructure-20260605.md)
...
| 当前已有多条 phase 分支待合并 | R1 已通过 PR #270 合并；R2/R3/R4/R5/R6A 已推送到独立 draft PR。后续并行先做 base 同步、截图 QA、ready gate 或领取 R6B/R7，不要在同一批 Desktop shell / IM renderer 文件上重复分派 |
...
|---:|---|---|---|---|
| 1 | **真实 TeamRun E2E transcript**：两个真实 Runtime Profile 在同一 group/team run 中协作，含 Orchestrator route、子任务、产出聚合、失败/审批处理 | 证据链已合并，真实录屏待做 | AgentHub 最大差异化证据；没有 transcript 时"多 Agent 协作"仍像后端能力声明 | PR #270 已合并，含 Desktop bridge nested context 测试和证据链文档；还需录屏、截图、运行日志、事件导出 |
...
- 深色频道式参考图只吸收协作密度和任务队列信息架构，不改变 light-first 主方向：左侧 channel/team roster 可以作为 R7 dark screenshot 的候选，但核心必须是 AgentHub 自己的 `research-team`/TeamRun 证据，而不是 Discord/Slack 复刻。最新参考图进一步确认右侧不应只是静态 summary，而应采用 `Active / Done / Warning` segmented queue：每个 Agent 任务卡显示角色、当前 tool、进度、耗时、暂停/取消/审批/详情入口，排队任务不能和正在执行任务混在一起。
...
| R1 | TeamRun E2E transcript 与证据导出 | opus 设计，haiku/sonnet 按需实现 | Hub/Edge 最小修复、Desktop evidence UI、`docs/competition/*` | PR #270 已合并 Desktop bridge 与证据路径；最终仍以两个真实 Runtime Profile 录屏、截图、日志、事件导出为准 |
...
| `phase-r2/desktop-shell` | PR #271 已 ready；head `0048be84`；R2 已同步主线 CI 基线修复，GitHub `frontend-desktop` 与 `validate` 已绿；PR 仍被 repo-wide Web/Mobile/Go/Docker/E2E 红项阻塞 | `uiStore.test.ts` 4/4；Desktop `pnpm typecheck` 通过；`eslint src/App.tsx` 0 errors；Playwright console 无 ErrorBoundary；light 1440/1280/390 shell 截图已生成；Claude sonnet 只读 review PASS；队列 focused tests 128/128；`git diff --check` 通过；Actions `frontend-desktop` SUCCESS、`validate` SUCCESS | Desktop 冲刺不分散治理 Web/Mobile/Go/Docker/E2E；合并若被必需检查挡住，走明确豁免或单独 CI 基线队列；最终比赛截图仍需 R3/R4/R7 叠加真实 transcript、右侧证据列和 Demo 运行产物 |
...
| `phase-r6/im-optimistic` | draft PR #275 已开；integration dry-run 无冲突 | `useIMChat.test.ts` + `IMMessageView.test.tsx` 37/37；队列 focused tests 128/128；`git diff --check` 通过 | 只完成 R6A 乐观发送；approval 一致性另起切片；分支同步 base 后重跑 typecheck |
...
R2 已经达到本地代码 review 门槛，并已同步主线 CI 基线修复；PR #271 当前 Actions 中 `frontend-desktop` / `validate` 已通过。后续 Desktop agent 不要转去治理 Web/Mobile/Go/Docker/E2E：这些是 repo-wide 基线债，记录为合并策略/发布门禁问题，Desktop 主线继续按 R2 -> R4 -> R5 -> R3 -> R6A 收敛。
...
- Web/Mobile/Go/Docker/E2E：当前 CI 还有 web lint、mobile typecheck、edge test、hub unit test、hub docker build、E2E smoke 等红项；这些不是 R2 touched files 引起，比赛 Desktop 冲刺不接手，后续等 Desktop 定型后再统一迁移/治理。
