...
| 当前短切片 | `codex/artifact-lifecycle-next`：只补 Edge read-only metadata dereference：`GET /v1/artifacts/{artifactId}`、`GET /v1/previews/{previewId}`；不做 content/apply/discard、preview start/stop、Hub artifact store、Web 直连 Edge 或真实 CLI/model gate |
...
| 7 | Artifact/Diff/Preview 生产化 | read-only Edge API 首片已补 `GET /v1/runs/{runId}/diff`、`GET /v1/artifacts`、`GET /v1/previews`，runtime evidence 写入已合入；当前短切片补 artifact/preview metadata lookup，不做 preview start/stop 或 artifact content/apply/discard | Edge API、shared inspector、Desktop smoke；Web 仍不直连 Edge |
