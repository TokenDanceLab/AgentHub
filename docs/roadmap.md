...
| **Web** | React + Vite | WebAgent closeout 已合入 `dev/delicious233`；Hub message runtime payload 与 RunDetail projection 已接入真实 Hub blocks；旧 Trump/Web parity 分支只作单独审查输入 | `corepack.cmd pnpm test -- src/utils/hubAdapters.test.ts` + `corepack.cmd pnpm typecheck` + `corepack.cmd pnpm exec vite build` 通过 | 不做硬性要求；根/wrapper `pnpm build` 在 Windows Node/libuv 生命周期上仍按既有债务单独处理 |
...
- [ ] Web follow-up：继续把 Settings/Agent Market/Execution Target 面从 preview fallback 收敛到 Hub Agent Profile、registered Edge target 和 Hub task lifecycle；公开 Web 发布前补 BFF/HttpOnly cookie 或等价 server-owned session。
- [ ] Runtime history follow-up：参考 OpenHands event service、Dify workflow logs、LangGraph thread/run inspection 和 OpenCode 本地 server/session 模式，Hub 侧补 `taskId -> edgeRunId -> typed RunEvent projection`，让 Web/Home 从 Hub 展示 run history、状态筛选、token/step/elapsed/approval/artifact 摘要；Edge 仍保留完整本地 EventStore，前端只消费 AgentHub 事件族，不直读 Codex/Claude Code/OpenCode 私有 JSON。bytedance.md 功能差距和竞品方向见 `docs/reference/cross-comparison/11-bytedance-feature-map.md`。
