...
| 已合入主干 | shared v4 workbench、Web Hub-only 主链路、Contacts/AgentProfile/Projects read-through、AgentProfile mutation、Hub Projects P1、ExecutionTarget contract、Edge pins/store/event contracts、Edge SQLite opt-in + relational migration、TeamRun fixture/dry evidence gate、Artifact/Diff/Preview read-only Edge 前置、runtime evidence 写入 |
| 当前短切片 | `codex/artifact-content-contract`：artifact content readiness proposal/test-only；确认当前 Edge store 只有 sanitized metadata path，没有可安全解析的 content source，因此 `GET /v1/artifacts/{artifactId}/content` 继续 planned/404；不做 content/apply/discard、preview start/stop、Hub artifact store、Web 直连 Edge 或真实 CLI/model gate |
...
| 当前候选切片 | `codex/desktop-target-tauri-host`：Desktop-owned Local Edge target preference 与 Tauri host readiness，待 rebase/verify/merge；`codex/runtime-evidence-inspector`：shared RightInspector read-only 消费 Edge runtime evidence snapshot，已 rebase 到 `9bf674f0` 后补 review blocker 修复，并通过 focused gate 复验，待 review/merge；`codex/artifact-lifecycle-next`：Edge artifact/preview metadata lookup，待 focused gate/merge；`codex/artifact-content-contract`：proposal/test-only content readiness |
...
| 7 | Artifact/Diff/Preview 生产化 | read-only Edge API 首片已补 `GET /v1/runs/{runId}/diff`、`GET /v1/artifacts`、`GET /v1/previews`，runtime evidence 写入与 metadata lookup 已合入候选；artifact content readiness 已确认缺 content source 字段与 run/workspace root 绑定，先保持 planned/404，不做 preview start/stop 或 artifact content/apply/discard | Edge API、shared inspector、Desktop smoke；Web 仍不直连 Edge |
...
5. `codex/runtime-evidence-inspector` 已补 shared RightInspector read-only snapshot 消费和 Desktop 既有 Edge evidence hook 接线，并修复 raw run id 选择、Desktop App v4 测试 harness、artifact metadata 非交互行与 preview 切 tab 行为；`codex/artifact-lifecycle-next` 只补 Edge artifact/preview metadata lookup；`codex/artifact-content-contract` 只记录 content readiness blocker 并用测试保持 planned/404。三者都不碰 preview start/stop、artifact apply/discard、Web 直连 Edge、Hub route 或真实 CLI/model。
