...
- [x] 2026-05-26 Hub→Desktop→Edge 真实启动 payload 补强：Hub `agent.dispatch` 现在从触发消息 JSON 提取 `prompt`；Web 自动邀请 Claude Profile 时使用 Edge adapter id `claude-code`；Desktop bridge 规范化 legacy `claude` 别名，并在 `POST /v1/runs` 前先 `POST /v1/threads` 确保 Hub session 对应的 Local Edge thread 存在。验证：`hub-server && go test ./internal/service -run "Test(PromptFromMessage|DispatchTaskIncludesPrompt|TriggerAgentTask|HandleTaskAck|HandleTaskDone|HandleTaskFail)" -count=1`、`app && corepack.cmd pnpm --filter agenthub-desktop exec vitest run src/__tests__/useHubIntegration.test.ts`、Web/Desktop production typecheck 通过；Desktop 全量测试 typecheck 仍有既有 test-only strict optional/index 债。
...
- [ ] Web follow-up：清理 `app/web/src/hooks/useHubIntegration.ts`、`eventClient.ts`、旧 `useChatMessages.ts` 等未接主入口的直连 Edge 遗留面，或用静态检查阻断 Web 生产入口主动调用 `/v1/runs` / `127.0.0.1:3210`。
