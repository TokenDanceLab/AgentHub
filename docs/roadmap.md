...
- [x] Web follow-up：已删除 `app/web/src/hooks/useHubIntegration.ts`、`eventClient.ts`、`useEventStream.ts`、旧 `useChatMessages.ts`、`useRunners.ts` 等未接主入口的直连 Edge 遗留面，并新增 `scripts/verify-web-hub-boundary.ps1` 阻断 Web 生产入口主动调用 `/v1/runs`、`/v1/events` 或 `127.0.0.1:3210`。验证：`.\scripts\verify-web-hub-boundary.ps1`、`app/web && corepack.cmd pnpm typecheck`、`app/web && corepack.cmd pnpm build` 通过。
- [ ] Runtime history follow-up：参考 OpenHands event service、Dify workflow logs、LangGraph thread/run inspection 和 OpenCode 本地 server/session 模式，Hub 侧补 `taskId -> edgeRunId -> typed RunEvent projection`，让 Web/Home 从 Hub 展示 run history、状态筛选、token/step/elapsed/approval/artifact 摘要；Edge 仍保留完整本地 EventStore，前端只消费 AgentHub 事件族，不直读 Codex/Claude Code/OpenCode 私有 JSON。
