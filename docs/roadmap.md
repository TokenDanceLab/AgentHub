...
| P0-3 | Desktop v4 shell 接入 | 进行中 | 接入附件/workdir/approval mode UI、真实 preview 打开能力，继续替换剩余静态 smoke 数据 | Desktop App v4 focused tests 4 passed；Desktop typecheck/build 通过；1440x920 Playwright smoke 通过，且读取真实 Edge thread 列表、persisted items、live Edge event evidence，可从 v4 composer 提交 Edge run，并在 shared inspector 展示 run/tool/file/artifact evidence |
