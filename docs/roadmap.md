...
- [ ] M2 Edge 本地数据层：Project / Thread / Run / Item / EventStore。最小内存实现已在 PR #30，message/item 写入链路、Runner lifecycle 边界、store 接口边界、轻量 JSON 文件持久化实现、`--store-file` 启动参数和 P0 IM OpenAPI typed schema 已补齐，SQLite 仍是后续可选评估项。
...
- [x] 细化 P0 Project / Thread / Item REST API 的 OpenAPI typed schema，保持路径、operationId 和事件名不变。
