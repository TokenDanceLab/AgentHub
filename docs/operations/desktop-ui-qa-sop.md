...
5. `docs/handoff/STATE.md` 记录脱敏证据：截图路径、命令结果、手动点击路径和剩余风险。
6. 所有启用按钮都必须真实有效；未接线功能必须禁用、隐藏或展示明确不可用/恢复态，不能保留 no-op 控件。

## 1. 接手检查

1. 先读 `AGENTS.md`、`docs/handoff/STATE.md`、`docs/operations/client-roadmap.md` 和本文件。
...
7. 在 `docs/handoff/STATE.md` 写脱敏证据：
   - `.tmp/...` 截图路径；
...
- 未解决控件写入 `docs/handoff/STATE.md`，标清文件 owner 和下一步验证方式。
...
Desktop 证据建议按这个结构写入 `docs/handoff/STATE.md`：
...
停止或交接 Desktop UI 工作前，更新 `docs/handoff/STATE.md`：
