...
| **Mobile RN** | Phase 1 已让 Mobile required gate 通过：Expo Web Visual QA + mock-Hub 合同 + Expo Doctor；native/device/真实 Hub 登录仍需独立证据 | `corepack pnpm --dir app/mobile-rn verify:qa` |
...
- ✅ Hub contracts aligned；Mobile required gate 已通过 Expo Web Visual QA、mock-Hub 合同和 Expo Doctor。Android/iOS development build、设备权限、SecureStore、通知和真实 Hub 登录仍未在本仓库证据中证明。
...
- [x] sidecar 正确放置 | 验证人：`verify-tauri-package-dry.ps1`
- [x] Mobile required check 通过；native/device/真实 Hub 登录不在该 gate 证明范围 | 验证人：`corepack pnpm --dir app/mobile-rn verify:qa`
