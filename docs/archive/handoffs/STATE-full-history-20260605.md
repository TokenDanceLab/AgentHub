...
## 本次 Desktop/UI 推进（2026-06-05）

> 2026-06-05 Desktop Settings Agent Profile layout cleanup：本轮只修复 `app/desktop/src/components/settings/sections/AgentProfilesSection.tsx`、`app/desktop/src/components/settings/primitives/primitives.module.css` 和 Desktop zh/en i18n。Agent Profile 分区原先把 CSS Modules 类写成裸字符串 `summaryGrid/profileGrid`，导致卡片网格间距没有生效；本轮改为绑定 primitives module，并为 Agent Profile 使用 14px 的更疏卡片间距。Local Agent Profile 展开编辑器原先缺少 `profileEditor*` / `profileTool*` 样式，textarea、工具权限、select 和 slider 退回浏览器默认布局，造成截图中的灰块和控件重叠；本轮补齐编辑器文档流、工具矩阵、模型覆盖、slider、附件行、操作区和窄屏换行样式，并补齐 `settings.tool.*` 与 `settings.agentProfile*` 中英文文案，避免 raw i18n key。验证通过：`app/desktop && corepack.cmd pnpm typecheck`、`python -c "import json; [json.load(open(p, encoding='utf-8')) for p in ['app/desktop/src/i18n/locales/zh.json','app/desktop/src/i18n/locales/en.json']]; print('json ok')"`、`corepack.cmd pnpm exec node .tmp\agent-profile-layout-qa.mjs` 桌面 viewport、`$env:AGENT_PROFILE_QA_WIDTH='375'; $env:AGENT_PROFILE_QA_HEIGHT='900'; corepack.cmd pnpm exec node .tmp\agent-profile-layout-qa.mjs` 窄屏 viewport；probe 断言 `scrollWidth=innerWidth`、`minCardGap=14`、`firstGridDisplay=grid`、`editorDisplay=flex`、`textareaCount=1`、`selectCount=3`、`rangeCount=3`、`rawSettingsKeyVisible=false`、`overlapCount=0`、console 为空。截图和 probe：`.tmp/agent-profile-layout-collapsed.png`、`.tmp/agent-profile-layout-expanded.png`、`.tmp/agent-profile-layout-probe.json`，均为本地证据不入仓库。

> 2026-06-05 Desktop Settings Agent Profile model copy follow-up：延续上一条布局修复，只改 Desktop Settings 的 Agent Profile 卡片、`settingsShared.ts` 模型选项和 zh/en i18n。折叠态摘要不再展示 `Runtime adapter`、`Provider:`、`localStorage`、“后续绑定”或“展开可修改”类说明，改为只显示“当前模型 / 模型服务 / 使用全局模型设置或此 Profile 单独设置 / 当前设备运行”。展开态的模型区域明确是“默认模型”，并动态把当前路由模型补进下拉选项；因此 `newapi/deepseek-v4-pro` 这类当前显示的模型也可被用户选择。验证通过：`app/desktop && corepack.cmd pnpm typecheck`、zh/en JSON parse、桌面和 375px viewport 的 `.tmp\agent-profile-layout-qa.mjs`，并额外 Playwright 文本探针确认页面正文 `localStorage=false`、`Runtime adapter=false`、`Provider:=false`、`后续会/will bind/raw Runtime=false`、`openCodeModelOption=true`。
...
- Desktop：项目文档后台 sweep 已完成，`docs/architecture/system-design/system-architecture.md` / `docs/architecture/system-design/product-requirements.md` / `docs/architecture/system-design/implementation-guide.md` / `docs/tutorials/roadmap.md` / README 系列 / archive + ADR 索引已统一 Runtime/Profile/Configuration/Execution Target、TokenDance ID、Hub/Edge/Desktop/Web 边界。
...
- Docs：gpt-5.5 xhigh 文档架构 worker 已完成文档架构审查，结论已汇总到 roadmap/STATE；`docs/inbox/` 已删除，已处理报告归档到 `docs/reference/` 或 `docs/archive/`。主文档已基本对齐 Runtime/Profile/Configuration/Execution Target、TokenDance ID、IM、多端、远控、Skill/MCP、cc-switch、安全审计等边界；剩余风险集中在 `/v1/runners` / `runner.*` 的历史兼容命名，以及 `docs/archive/client-handoff.md`、`docs/tutorials/integration.md` 等旧独立 `runner/` 文档需要归档或改写。
...
- Docs：`bytedance.md` 功能映射已归档到 `docs/reference/competitive/11-bytedance-feature-map.md`。该文档只作为 gap matrix 和竞品方向参考，不是 Trump/旧 worktree 完成证据；下一批顺序仍是部署态 TokenDance ID smoke、Runtime UI parity、多 Agent group smoke、Artifact lifecycle，以及 Home 站点到 AgentHub Web/Hub session 的低风险深链。
...
- Docs：处理文档架构审查的最小建议：先在 API/架构文档标注 Runner 兼容命名，再归档或改写旧 `docs/archive/client-handoff.md`、`docs/tutorials/integration.md`。
- Docs：Codex follow-up 已完成，确认最小补丁应优先改 `docs/architecture/system-design/implementation-guide.md` 的旧 client 文档入口，并在 API docs 标注 `/v1/runners` / `runner.*` 是历史兼容命名。
...
- `docs/tutorials/roadmap.md` — 全局路线图（唯一事实源）
