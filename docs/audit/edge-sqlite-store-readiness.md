...
> Date: 2026-06-09
> Worker: Worker F
> Base: `origin/codex/p1-remote-control-evidence-integration`
> Scope: Edge durable SQLite store proposal and first safe slice. No Hub, Web, Desktop, CLI adapter execution, mobile, or roadmap files were changed.
...
- `SQLiteStore` wraps the in-memory `Store`, persists a full JSON snapshot to `agenthub_store_snapshots`, and rewrites relational projection tables after each successful write.
- Relational projection tables exist for owners, workspaces, runs, artifacts, diffs, and previews.
- Existing contract tests cover memory, file, and SQLite restore behavior; SQLite migration tests cover idempotent apply, rollback to snapshot-only schema, foreign keys, nested path creation, and transient lock waiting.

What is not proven:

- Row-level SQLite write authority for projects, threads, runs, items, pins, diffs, artifacts, or previews.
...
Do not claim production durable store readiness from the current state. The current branch proves a snapshot-backed SQLite preview and read-model projection only.
...
4. Add future row-first durable migrations after version 3 only.
5. Preserve rollback to version 1 for preview deployments until row-first migration has a tested export/import path.
...
| Reopen after each repository write type | Proves each write is independently durable, not only final close/flush state. |
...
1. Current `SQLiteStore` restore authority is still `agenthub_store_snapshots`, not relational tables.
...
3. Relational tables are projections owned by `agenthub_store_snapshot`; they are deleted and rebuilt on each persist.
...
This slice intentionally changes only this audit report. It records the current implementation state, the safe migration order, the required contract tests, and the blockers. It does not make a production durable-store claim.
