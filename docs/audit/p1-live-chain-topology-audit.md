...
|---|---|---|---|---|
| Web composer dispatch requires Hub and selected target | implemented/proven | `app/web/src/platform/webPlatform.ts`: `createWebPlatform`, `submitWebComposerIntent`, `resolveWebDispatchTarget`, `triggerMentionedAgent`; Hub endpoint `POST /web/agent-tasks` via `app/web/src/api/hubClient.ts:triggerAgentTask` | `app/web/src/platform/webPlatform.test.ts` checks `target_id` is sent and dispatch failure is surfaced. This audit's verifier checks production Web source for no direct `127.0.0.1:3210`, `/v1/runs`, `/v1/events`, legacy Edge hooks, or Desktop/Tauri imports. | Keep Web-only dispatch work in `app/web/src/platform/webPlatform.ts` and `app/web/src/api/hubClient.ts`; do not add browser calls to Local Edge. |
| Web TeamRun target selection | implemented/proven | `app/web/src/views/TeamRunConsole.tsx`: `useHubExecutionTargets`, `handleStartRun`, request body `{ trigger_message, target_id }`; `app/web/src/api/hubClient.ts:startTeamRun` uses `POST /web/agent-teams/{id}/runs` | `app/web/src/views/TeamRunConsole.test.tsx` checks selected online `local_edge` target id, no-start when no online `local_edge`, target switching, and dispatch-denied errors. | Keep TeamRun launch edits in `app/web/src/views/TeamRunConsole.tsx`; expand tests there when adding target UX states. |
| Hub route surface for Web tasks, target inventory, TeamRun, and Edge callbacks | implemented/proven | `hub-server/internal/router/router.go`: `/web/agent-tasks`, `/web/execution-targets`, `/web/agent-teams/...`, `/edge/devices/register`, `/edge/agent-tasks/:id/{ack,stream,done,fail}`. Handlers: `hub-server/internal/handler/agent.go`, `agent_team.go`, `execution_target.go`, `device.go` | `hub-server/internal/handler/device_openapi_test.go` verifies OpenAPI/router route parity and device-type metadata. `hub-server/internal/handler/edge_protocol_test.go` covers device registration, Web trigger, target id pass-through, callbacks, and event summary. | Route additions must update `hub-server/internal/router/router.go`, handler tests, and `api/openapi.yaml` together. |
...
- Hub route and handler symbols for Web tasks, execution targets, TeamRun, Edge device registration, and Edge task callbacks exist.
- Hub dispatch validation still routes Web tasks only through target-bound Desktop `local_edge` inventory.
- Web composer and TeamRun still require selected online `local_edge` targets and send `target_id`.
