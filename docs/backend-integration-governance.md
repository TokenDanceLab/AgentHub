...
7. **D1b/D2/D3. 剩余 CI / release / real CLI gates**：延后。D1b 只在新增依赖测试已进入主线且不会让 CI 变红时再拆；`release.yml` 是发布流程变更；`real-cli-e2e.yml` 只能在专用 runner、环境审批、预算和 artifact 脱敏确认后 opt-in/nightly。
   ...
   - **D2b. Release dry build topology**：本片只做 topology/preflight only（拓扑/预检）边界，不把完整 Tauri 构建当成静态 readiness 通过条件，也不声明当前 dry topology 会产出安装器或 updater 文件。静态 gate 只验证 `release-readiness.yml` 把 Windows sidecar `agenthub-edge-x86_64-pc-windows-msvc.exe`、Windows unsigned NSIS/portable artifact 命名、`latest.json`/`.sig` updater metadata、ignored 输出路径、workflow artifact 上传边界和 `TAURI_SIGNING_PRIVATE_KEY` 禁用边界写清楚；`pnpm tauri build` / full Tauri build 只能留在单独的 `workflow_dispatch` + `run_windows_package_dry` 手动 opt-in job，不能被静态 policy 或 installer smoke preflight 代替审批。macOS 下一步只允许新增 `macos-latest` / arm64 unsigned validation：准备 `agenthub-edge-aarch64-apple-darwin` sidecar，验证 Tauri bundle 路径、DMG/app 产物命名和 artifact 上传边界；不得运行 Developer ID signing、`codesign --sign`、`xcrun notarytool`、`stapler staple` 或任何发布创建。正式签名、notarization、staple、release asset 上传和 updater 生产 metadata 发布必须另起 proposal，并要求专门审批、secret 边界和脱敏证据。
...
27. **Artifact content contract readiness**：`codex/artifact-content-contract` 只做 proposal/test-only 收口。当前 Edge `store.Artifact` 只有 `id/runId/threadId/kind/path/sizeBytes/createdAt/updatedAt`，runtime evidence writer 会把绝对路径净化成 workspace-relative 或 basename-only `path`，但 `Run` 不持久化 `workDir`，artifact 也没有 `workspaceId`、blob key、content path、MIME type、checksum、preview-safe size cap 或 readable content root。因此本片不实现 `GET /v1/artifacts/{artifactId}/content`，继续用测试锁定 OpenAPI `planned` 与 HTTP 404 行为。下一片最小实现范围应先落 `edge-server/internal/store/store.go`、`file_store.go`、`sqlite_store.go`、`sqlite_migrations.go` 的 artifact content source 字段与 memory/file/sqlite contract，再改 `edge-server/internal/lifecycle/runtime_evidence.go` 写入安全 content source，最后补 `edge-server/internal/api/handlers.go` 只读 content handler 和 `api/openapi.yaml` 响应 schema。仍不得做 apply/discard、Hub artifact store、Web 直连 Edge、Desktop UI 或真实 CLI/model gate。
...
| RightInspector | 默认任务/文件内容仍主要来自 demo/event evidence；Edge 已补 artifact/diff/preview read-only REST 空态/数据合同，runtime evidence 写入已收口，`codex/runtime-evidence-inspector` 已在 shared inspector 增加 read-only snapshot 消费并由 Desktop 复用既有 Edge evidence hook；review blocker 修复覆盖 raw run id、Desktop App v4 harness、artifact metadata 非交互行和 preview 切 tab，已通过 focused gate 复验；`codex/artifact-lifecycle-next` 补单条 artifact/preview metadata lookup；`codex/preview-lifecycle-readiness` 补 stopped metadata transition；preview start/runner、artifact content/apply/discard 和真实 CLI evidence 仍未完成 | Review/merge runtime evidence inspector、metadata lookup 和 stopped transition；下一片 preview runner 只允许 fake runner + Edge API/store/OpenAPI focused tests，artifact apply/discard/content 另拆 |
...
3. 下一批实现按 roadmap 排序：登录 fake/local gate、Edge SQLite opt-in backend、Edge relational migration、Tauri Windows installer/updater metadata readiness、TeamRun dry fixture evidence、Artifact/Diff/Preview read-only endpoints、runtime evidence metadata 写入和 artifact/preview metadata lookup 已合入；`codex/tauri-package-readiness` follow-up 已补 generated artifact ignore gate，待 review；`codex/tauri-installer-smoke` 补 Windows installer smoke preflight，作为完整 dry package build 前的轻量本地/CI 先决条件检查；`codex/desktop-target-tauri-host` 正在收口 Desktop-owned Local Edge preference 与 Tauri host readiness；`codex/runtime-evidence-inspector` 已补 shared inspector read-only snapshot 消费并完成 review blocker 修复和 focused gate 复验，待 review/merge；`codex/preview-lifecycle-readiness` 只补 preview stopped metadata transition。下一批继续推进 Projects create/update UI、packaged login E2E；Projects delete/soft-delete policy、preview start/runner、artifact content/apply/discard、macOS signing/notarization 和 D1b/D2/D3 gate policy 继续保持独立 proposal。
