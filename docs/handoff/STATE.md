...
- Worktree 建议：下一轮优先拆 `feat/runtime-event-blocks-ui`（typed blocks/tool group/artifact projection）、`feat/operational-home-console`（两端 Home 展示 active runs、pending approvals、target health、TokenDance ID session state）和 Execution Target dispatch 三段后端/runtime 任务。Execution Target 不要一口吃成远程/云执行：先做 `feat/hub-target-id-dispatch-contract` 的 owner/workspace allowlist 契约，再做 `feat/hub-edge-target-routing` 的 target-bound device route 和 pending queue 隔离，最后做 `feat/runtime-typed-control-callbacks` 的 normalized callback/control event。所有任务都必须保持 Web Hub-only、Hub-issued session 授权和 secret-free 证据边界。
