...
- Runtime readiness：`.\scripts\verify-runtime-readiness.ps1` 是 AgentAdapter/Runtime wiring 的结构 gate，只证明仓内真实调用链和 UI/docs 边界对齐；下一步需要分别做 Codex、Claude Code、OpenCode live smoke 和截图/事件证据，尤其 Claude Code 阻塞式审批不要用当前 permission event 展示替代。
