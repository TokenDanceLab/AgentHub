...
### 当前接手顺序（2026-05-26）
- Runtime bridge：Web→Hub task→Desktop→Local Edge→Hub stream/done 的可见输出闭环已落地并有回归测试。下一步不要重复补这一层；优先做真实 TokenDance ID 部署态登录/设备注册 smoke、Desktop logout/reconnect 截图证据、以及 Web 公开发布前的 BFF/HttpOnly cookie 或等价 server-owned session 方案。
