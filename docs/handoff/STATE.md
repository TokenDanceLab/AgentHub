...
> 2026-06-05 Desktop Settings i18n cleanup：从 `dev/trump` 新建 `feat/desktop-settings-i18n-cleanup`，只修复 Desktop Settings 的 zh/en 字典和少量已接入 i18n 的设置页组件文案。补齐 `settings.language`、`settings.data*`、`settings.status.*`、Agent 创建器、Agent 市场向导、Data 页分类和发布预览工具计数等缺失 key；保留 Agent、Hub、MCP、Web、Windows、cc-switch 等产品/平台名。验证通过：zh/en JSON parse、静态扫描 `settings.*` 缺失 key 结果为空、`app/desktop && corepack.cmd pnpm typecheck`，以及 Playwright 中文页面探针逐个点击 31 个 Settings 导航项，结果 `rawCount=0`、`settings.language=false`、`settings.data=false`、`settings.status.loginLocked=false`、console 为空。
