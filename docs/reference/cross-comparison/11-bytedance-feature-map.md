...
| Claude Code adapter | 已实现，有 caveat | `edge-server/internal/adapters/claude_code.go`；live smoke 记录在 `docs/development/handoffs/STATE.md` | 阻塞式 human-in-the-loop stdin `can_use_tool` 回写仍是单独安全验收。 |
...
| Desktop 本地文件/进程管理 | 部分完成，Desktop QA 增强 | Desktop -> Local Edge -> Runtime adapters 是当前最强实现链路；本轮补了模型 catalog、附件、工作区选择、自定义指令、真实 Claude Code context QA 和客户端截图证据 (14 screenshots)，详见 `docs/development/handoffs/STATE.md` | Packaged TokenDance ID login/logout/reconnect、TeamRun Console 和跨端 Hub smoke 仍未关闭。 |
