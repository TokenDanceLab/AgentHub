...
> 它用于描述中心 IM、好友、群聊、Agent 元数据、消息和任务路由需求；具体工程结构、数据库和消息队列选型仍需继续对齐 `docs/system-architecture.md`、`docs/implementation-guide.md` 和 `api/`。
...
本节为 `server—hub` 分支提交的技术栈建议，供后续实现参考；最终选型以 `docs/system-architecture.md`、`docs/reference/03-build/backend/02-go-services.md` 和实际工程约束为准。
