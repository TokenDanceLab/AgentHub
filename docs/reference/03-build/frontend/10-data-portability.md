...
> 基于: design-eventstore-memory.md (JSONL SSOT), deep-dive-librechat-message-tree.md (Fork/UUID 重生成),
...
**核心原则**: JSONL 是 SSOT。所有导出格式从 JSONL 派生，所有迁移以 JSONL 为最小可传输单元。
...
| 决策 | 选择 | 依据 |
|------|------|------|
| 导出 SSOT | JSONL | 与 EventStore 同格式；导入即回放；增量同步天然支持 |
