...
本目录保存已采纳的架构决策记录。ADR 解释“为什么当时这样决定”，不自动覆盖当前主文档。若 ADR 与 `docs/system-architecture.md`、`docs/implementation-guide.md` 或 `api/` 契约冲突，以当前主文档和契约为准，并在后续新增 ADR 记录变更原因。
...
| `ADR-004-go-process-orchestration.md` | 已采纳，当前执行边界权威。 | Go `os/exec` + AgentAdapter 是当前独立 Runner 替代方案。它应与 `docs/system-architecture.md` 的 Agent Runtime/Profile/Execution Target 术语一起阅读。 |
