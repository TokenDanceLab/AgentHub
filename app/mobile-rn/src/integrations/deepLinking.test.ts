...
describe('AgentHub mobile deep links', () => {
  ...
  it('maps AgentHub app deep links to navigation targets', () => {
    ...
    expect(parseAgentHubDeepLink('agenthub://run/run-tokendance?threadId=thread-delicious233')).toEqual({
      kind: 'navigate',
      target: {
        screen: 'run',
        runId: 'run-tokendance',
        threadId: 'thread-delicious233',
      },
    });
    expect(
      parseAgentHubDeepLink('agenthub://approval/approval-token-dance?runId=run-tokendance&threadId=thread-delicious233'),
    ).toEqual({
      kind: 'navigate',
      target: {
        screen: 'approval',
    ...
    expect(parseAgentHubDeepLink('agenthub://activity/activity-token-dance?runId=run-tokendance')).toEqual({
      kind: 'navigate',
      target: {
        screen: 'activity',
  ...
  it('starts a deep link bridge from initial URL and listener events', async () => {
    ...
    listener?.({ url: 'agenthub://run/run-tokendance?threadId=thread-delicious233' });
