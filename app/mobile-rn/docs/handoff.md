...
| Android dev-build proof | `corepack pnpm --filter agenthub-mobile-rn android` or `npx eas-cli@20.1.0 build --profile development --platform android` | Install and open the development build before claiming device proof. `native:check` is config readiness only. |
