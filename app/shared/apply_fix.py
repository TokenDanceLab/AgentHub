#!/usr/bin/env python3
"""Replace vi.mock('react-i18next') with vi.hoisted() version.

Reads the entire file, finds the vi.mock block (from 'import { sharedWorkbenchResources'
to the next 'vi.mock('"@lobehub/icons'"), and replaces it with a self-contained
vi.hoisted approach that does not reference any external imports.
"""
import sys
...
block_start = content.find("import { sharedWorkbenchResources } from '../i18n/workbench';")
next_mock = content.find("vi.mock('@lobehub/icons'")
before_next = content.rfind("\n", 0, next_mock)
block_end = before_next + 1  # keep the newline

if block_start < 0 or block_end <= block_start:
    print(f"ERROR: block_start={block_start}, block_end={block_end}")
    sys.exit(1)
