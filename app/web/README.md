...
`vite.config.ts` 使用固定端口 `5174` 和 `strictPort: true`。Desktop/Tauri 前端固定使用 `5173`，Mobile RN Expo Web 视觉预览固定使用 `5177`。端口被占用时先关闭旧服务，再重新启动。
