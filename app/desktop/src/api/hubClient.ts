...
// - Add shared methods/types first; desktop may only keep thin host glue until T3.3 cutover.
// - Mobile already uses shared re-export; desktop/web full forks are transitional.
...
export function createHubClient(opts: HubClientOptions = {}) {
  ...
  return {
    ...
    addMessageReaction: (messageId: string, sessionId: string, reaction: { emoji: string }) =>
      request<EmptyHubResponse>(`/client/messages/${encodeURIComponent(messageId)}/reactions`, {
        method: 'POST',
