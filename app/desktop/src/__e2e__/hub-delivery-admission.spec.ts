  ...
  await page.route('**/*', async (route) => {
    ...
    if (url.origin === 'http://127.0.0.1:3210') {
      if (url.pathname === '/v1/health') {
        ...
        if (calls.abortRun) { await route.abort('failed'); return; }
        if (calls.rejection) {
...
for (const theme of ['light', 'dark'] as const) {
  test('Desktop bridge delivers full execution intent and keeps edge-owned replay local (' + theme + ')', async ({ page, baseURL }) => {
    ...
    // A contradictory owner is not a transport replay: keep the original
    // mapping and do not acknowledge the conflicting receipt.
    calls.owner = 'desktop';
    await dispatch();
...
for (const theme of ['light', 'dark'] as const) {
  test('Desktop keeps unconfirmed run POSTs pending for review (' + theme + ')', async ({ page, baseURL }, testInfo) => {
    if (!baseURL) throw new Error('Desktop E2E baseURL is required');
    const { calls, dispatch } = await installDispatchFixture(page, baseURL, theme);
    calls.abortRun = true;
    await dispatch(false);
    await expect.poll(() => calls.runs.length).toBe(1);
    await expect.poll(() => readTaskError(page)).toContain('uncertain');
    await expect(page.getByText(/Edge admission result is uncertain/)).toBeVisible();
    await expect.poll(() => readTaskState(page)).toEqual({ tasks: [{ taskId: TASK, status: 'queued', runId: null }], runToTask: {} });
    expect(calls.acks).toHaveLength(0);
    expect(calls.fails).toHaveLength(0);
    expect(calls.relayAcks).toHaveLength(0);
    await page.screenshot({ path: testInfo.outputPath('unconfirmed-post-' + theme + '.png') });

    calls.abortRun = false;
    calls.rejection = { status: 500, code: 'internal_error' };
    await dispatch();
    await expect.poll(() => calls.runs.length).toBe(2);
    await expect.poll(() => readTaskState(page)).toEqual({ tasks: [{ taskId: TASK, status: 'queued', runId: null }], runToTask: {} });
    ...
    // Only another Hub delivery reconciles the same admission; no local retry.
    calls.rejection = null;
    await dispatch();
    await expect.poll(() => calls.runs.length).toBe(3);
    await expect.poll(() => calls.acks.length).toBe(1);
