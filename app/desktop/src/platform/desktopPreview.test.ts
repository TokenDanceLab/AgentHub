...
import { resolveDesktopEvidenceContentUrl } from './desktopPreview';

describe('resolveDesktopEvidenceContentUrl', () => {
  it('returns absolute evidence URLs unchanged', () => {
    expect(resolveDesktopEvidenceContentUrl('http://127.0.0.1:4173/preview'))
      .toBe('http://127.0.0.1:4173/preview');
    expect(resolveDesktopEvidenceContentUrl('https://preview.example.com/app'))
  ...
  it('resolves Edge-relative content paths against the Local Edge base URL', () => {
    ...
    expect(resolveDesktopEvidenceContentUrl('/v1/runs/run-1/artifacts/artifact-1/content'))
      .toBe('http://127.0.0.1:3210/v1/runs/run-1/artifacts/artifact-1/content');
    expect(resolveDesktopEvidenceContentUrl('/v1/runs/run-1/previews/preview-1/content'))
      .toBe('http://127.0.0.1:3210/v1/runs/run-1/previews/preview-1/content');
  });
