// Package attachment owns IM attachment metadata + blob orchestration for Hub.
//
// It is the fourth IM typed-service package (agentteam-style; #697), extracting
// the attachment domain from the flat service package. Pure hash/path/metadata
// helpers remain in service/im (#628); ObjectStorage port ownership originates
// from the #606 thin first seam.
package attachment
...
// Service owns attachment metadata + blob orchestration: hash probe/dedup save,
// blob put/get/delete/presign, active session-member access check, and upload
// mime/size policy. Blob I/O goes through the injected ObjectStorage port.
type Service struct {
	db        *gorm.DB
...
// NewService constructs an attachment Service.
// storage may be nil for metadata-only/partial tests; blob paths error or no-op.
func NewService(db *gorm.DB, uploadCfg config.UploadConfig, storage ObjectStorage) *Service {
	return &Service{db: db, uploadCfg: uploadCfg, storage: storage}
}
...
func (s *Service) storagePort() ObjectStorage {
	if s == nil {
		return nil
	}
	return s.storage
...
func (s *Service) ProbeAttachment(ctx context.Context, userID, hash string) (*model.Attachment, error) {
	if userID == "" {
		return nil, nil
	}
...
func (s *Service) SaveAttachment(ctx context.Context, uploaderID, hash, mimeType, originalName string, size int64) (*model.Attachment, error) {
	return s.SaveAttachmentWithMetadata(ctx, uploaderID, hash, mimeType, originalName, size, "")
}

func (s *Service) SaveAttachmentWithMetadata(ctx context.Context, uploaderID, hash, mimeType, originalName string, size int64, metadata string) (*model.Attachment, error) {
	if !im.IsValidAttachmentHash(hash) {
		return nil, errcode.ErrBadRequest
	}
	normalizedMetadata, err := im.NormalizeAttachmentMetadataJSON(metadata)
	if err != nil {
...
func (s *Service) DeleteBlob(ctx context.Context, hash string) error {
	key := im.PathFromHash(hash)
	if key == "" {
		return nil
	}
...
func (s *Service) BlobLocalPath(hash string) string {
	key := im.PathFromHash(hash)
	if key == "" {
		return ""
	}
...
func (s *Service) PresignBlobURL(ctx context.Context, hash string, contentType string, contentDisposition string) (string, error) {
	key := im.PathFromHash(hash)
	if key == "" {
		return "", nil
	}
...
func (s *Service) GetAttachmentByID(ctx context.Context, userID, id string) (*model.Attachment, error) {
	if userID == "" {
...
func (s *Service) IsAttachmentMimeTypeAllowed(mimeType string) bool {
