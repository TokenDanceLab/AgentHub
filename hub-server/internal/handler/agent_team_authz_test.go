...
// newAuthzRouter builds a test router with the same route layout as production.
// Middleware injects user_id from the X-Test-User header.
func newAuthzRouter(h *AgentTeamHandler) *gin.Engine {
...
// member receives 403 on every gated endpoint. Uses a real AgentTeamService
// backed by the in-memory SQLite DB so the authz logic (now in the service
// layer) is exercised end-to-end.
func TestAuthz_NonMemberForbidden(t *testing.T) {
	db := setupAuthzTestDB(t)
	seedTeamFixture(t, db)
	svc := agentteam.NewAgentTeamService(db, nil, nil)
...
func TestAuthz_OwnerPasses(t *testing.T) {
	...
	// UpdateTeam: owner passes authz → service may return bind error (no body)
	// but must NOT be 403.
...
// needed by the service-layer authz logic.
