...
这会启动 PostgreSQL、Redis 和 server-hub 三个容器，服务监听 `localhost:8080`。
...
│   ├── errcode/                  # 错误码定义
│   ├── jwtutil/                  # JWT 生成/解析
│   ├── cache/                    # Redis 客户端 + 设备路由
│   ├── ws/                       # WebSocket 连接管理 + 帧协议
│   ├── middleware/               # JWT 认证 + 设备类型校验
│   ├── router/                   # 路由注册
...
内存事件总线（pub/sub）解耦持久化与 WebSocket 推送：
...
消息的 seq_id 是会话级别自增，通过 PostgreSQL 原子操作保证并发安全：

```sql
UPDATE sessions SET next_seq = next_seq + 1, last_message_at = NOW()
